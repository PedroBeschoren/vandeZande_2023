...
insect_ps<-subset_samples(insect_ps, Observation %in% c(4,16))
...
ps_list_network_insect <-insect_ps_soil_l
...
pargs <- list(rep.num = 75, seed = 10010, ncores = 1, thresh = 0.05)
...
list_spiec_objct <- lapply(ps_list_network, function(x) {
  SpiecEasi::spiec.easi(x,
    ...
    lambda.min.ratio = 1e-3, # the higher the more edges, leading to higher density network. as it goes lower, computational times increases SUBSTANTIALLY. standard is 1e-3 but 1e-2 works well ; pedro's PC crahs when 1e-7
    nlambda = 20, # number of slices between minimum amount of edges to maximum. low lavues dive more sparse networks, high values give denser networks. keep it between 10 and 100
...
save(list_spiec_objct, file = "./Results/list_spiec_objct.RData")
...
#has to be tested with els' stuff
...
Pedro to do:
* change insect .csvfeature names from "(" and  "." to "_"
* match bacteria and isnect samples acoding plant number (above)
* only run networks for week 4 and week 16. in case f errors, focus on week 16. one model for pot, one model for field.
* run an RF model to find ASVs that separate time points. one model for pot, one model for field.
* run RF model to find ASVs that separate treatments. one model for pot, one model for field.
