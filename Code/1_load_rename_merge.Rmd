...
block_info<-
read.delim(file = "./Data/Metadata_field_herbivore_observationscsv_2021.csv",sep = ";")

metadata$Plant_Number<-
gsub(pattern = ".*_p", x =metadata$Sample_Name, replacement = "")%>%
  gsub(pattern = ".*_P", replacement = "")%>%
...
metadata$Harvest<-
gsub(pattern = "Week.", x =metadata$Time_point , replacement = "")%>%
  as.numeric()

block_info<-
select(block_info, -Observation, -Column, -Row, - Label )%>%unique()

metadata$Treatment<-
  gsub(pattern = "Control", x = metadata$Treatment, replacement = "CO")%>%
      gsub(pattern = "Manure", replacement = "MAN")%>%
      gsub(pattern = "Chitin", replacement = "CHI")


metadata_join<-
left_join(metadata, block_info, by = join_by("Plant_Number" == "Plant_Number", 
                                             "Treatment"=="Treatment", 
                                             "Soil_type" == "Soil",
                                             "Harvest" == "Harvest"))

rownames(metadata_join)<-rownames(metadata)

write.table(x = metadata, file = "./Data/Els_please_check_NAblocks.csv", sep = ";")
metadata_for_ps <-sample_data(metadata_join)



# add metadata to phyloseq object with updated taxa names
sample_data(physeq)<-metadata_for_ps
