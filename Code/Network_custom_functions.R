...
filterPhyseq <- function(physeq, abund, freq){
  ...
  physeq_ra_filt = phyloseq::filter_taxa(physeq_ra, function(x) sum(x) > abund, TRUE)
  otu_table(physeq) <- otu_table(physeq)[which(
    rownames(otu_table(physeq))%in%rownames(otu_table(physeq_ra_filt))), ]
  physeq <- phyloseq::filter_taxa(physeq, function(x) sum(x >= 1) >= 
...
make_igraph<-function(spiec_obj, physeq_obj){
  ...
  secor  <- cov2cor(as.matrix(getOptCov(spiec_obj))) # silence this if using mb instead of glasso
  elist.gl <- multtest::summary(Matrix::triu(secor*getRefit(spiec_obj), k=1)) # for glasso
  network <- adj2igraph(Matrix::drop0(getRefit(spiec_obj)),
                        vertex.attr = list(name=names_spiec_obj),
                        edge.attr=list(weight=multtest::summary(Matrix::triu(t(secor)*getRefit(spiec_obj), k=1))[,3] ), # use this for glasso
